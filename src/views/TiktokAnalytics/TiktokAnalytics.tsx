...
function getCategories(t: (key: string) => string): { key: string; label: string }[] {
  return [
    { key: '', label: t('tiktok.cat.all') },
    { key: 'dance', label: t('tiktok.cat.dance') },
    { key: 'comedy', label: t('tiktok.cat.comedy') },
    { key: 'education', label: t('tiktok.cat.education') },
    { key: 'food', label: t('tiktok.cat.food') },
    { key: 'beauty', label: t('tiktok.cat.beauty') },
    ...
    { key: 'travel', label: t('tiktok.cat.travel') },
  ];
}

/* ── TikTok RPM rates ────────────────────────────────────────────── */

const TIKTOK_RPM: Record<string, number> = {
  dance: 0.02,
  comedy: 0.03,
  gaming: 0.05,
  minecraft: 0.06,
...
function UpgradeOverlay({
  ...
  return (
    <tr>
      <td colSpan={7} style={{ padding: 0, position: 'relative', border: 'none' }}>
...
export const TiktokAnalytics = memo(function TiktokAnalytics() {
  ...
  const [data, setData] = useState<TikTokItem[]>([]);
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.hideIndian) {
      result = result.filter((item) => !INDIAN_PATTERN.test(item.title) && !INDIAN_PATTERN.test(item.creator));
    }
    // Hashtag filter applied client-side for mock data
    if (filters.hashtag && isMock) {
      const q = filters.hashtag.toLowerCase().replace(/^#/, '');
    ...
    // Trending sounds filter
    if (filters.trendingSounds) {
      result = result.filter((item) => item.soundName);
    }
  ...
  const nicheStats = useMemo(() => {
    ...
    const rpm = TIKTOK_RPM[filters.category] ?? 0.03;
    const estimatedEarnings = (totalViews / 1000) * rpm;
    const engagementRate = totalViews > 0 ? ((totalLikes + totalComments) / totalViews) * 100 : 0;
    return { totalViews, totalLikes, estimatedEarnings, rpm, count: filteredData.length, engagementRate };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: C.bg, color: C.text, fontFamily: 'inherit' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4, flexWrap: 'wrap' }}>
            ...
            >
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path d="M11.5 1.5C11.5 1.5 11.5 5.5 11.5 6.5C11.5 9.26 9.26 11.5 6.5 11.5C3.74 11.5 1.5 9.26 1.5 6.5C1.5 3.74 3.74 1.5 6.5 1.5" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" fill="none" />
                <path d="M8.5 1.5V9.5" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" />
                <path d="M8.5 1.5C8.5 1.5 10.5 1.5 12 3C13.5 4.5 14.5 4.5 14.5 4.5" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" fill="none" />
              </svg>
            </div>
            <h1
              style={{
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
                  ...
                  <div style={{ padding: '8px 0', display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <div style={!isPro ? { opacity: 0.5, pointerEvents: 'none' } : undefined}>
                      ...
                      <input
                        ...
                        placeholder={t('tiktok.hashtagPlaceholder')}
                        style={{
                          width: '100%',
                          padding: '7px 10px',
                          borderRadius: 8,
                    ...
                    {/* Trending sounds toggle */}
                    <ToggleSwitch
                      checked={filters.trendingSounds}
                      onChange={(v) => setFilters((f) => ({ ...f, trendingSounds: v }))}
                      label={t('tiktok.trendingSounds')}
                      isDark={isDark}
                      accent={C.accent}
                      dim={C.dim}
                      sub={C.sub}
        ...
        <div style={{ flex: 1, overflow: 'auto', position: 'relative' }}>
          ...
          {nicheStats && !loading && (
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize: 22, fontWeight: 800, color: '#ff0050' }}>
                  {nicheStats.totalLikes >= 1_000_000_000
                    ? `${(nicheStats.totalLikes / 1_000_000_000).toFixed(1)}B`
                    : nicheStats.totalLikes >= 1_000_000
                      ? `${(nicheStats.totalLikes / 1_000_000).toFixed(1)}M`
                      : nicheStats.totalLikes.toLocaleString('ru-RU')}
                </div>
              </div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 11, color: C.dim, fontWeight: 600, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                  {t('tiktok.engagementRate')}
                </div>
                <div style={{ fontSize: 22, fontWeight: 800, color: C.accent }}>
                  {nicheStats.engagementRate.toFixed(1)}%
                </div>
          ...
          <div className="tf-tiktok-table-wrap" style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
          <table
            style={{
              width: '100%',
          ...
          >
            <thead>
              ...
              >
                {[t('tiktok.colRank'), t('tiktok.colVideo'), t('tiktok.colViews'), t('tiktok.colLikes'), t('tiktok.colComments'), t('tiktok.colShares'), t('tiktok.colCreator')].map((h, i) => (
                  <th
                    key={h}
                    style={{
                      padding: '12px 14px',
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} style={{ padding: '48px 14px', textAlign: 'center', color: C.sub, fontSize: 14 }}>
                    {t('tiktok.noData')}
                  </td>
                </tr>
              ) : (
                  ...
                  {visibleData.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                            ...
                            {item.thumbnail ? (
                              ...
                              >
                                ...
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" style={{ opacity: 0.3 }}>
                                  <path d="M9 18V5l12-2v13" stroke={C.sub} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
                                  <circle cx="6" cy="18" r="3" fill={C.sub} opacity="0.5" />
                                  <circle cx="18" cy="16" r="3" fill={C.sub} opacity="0.5" />
                                </svg>
                                {/* TikTok badge */}
                                <div
                                  style={{
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                              ...
                              {item.soundName && (
                                <span style={{ fontSize: 10.5, color: C.dim, display: 'flex', alignItems: 'center', gap: 3 }}>
                                  <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                                    <path d="M4 8V2l5-1v6" stroke={C.dim} strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" fill="none" />
                                    <circle cx="2.5" cy="8" r="1.5" fill={C.dim} opacity="0.6" />
                                    <circle cx="7.5" cy="7" r="1.5" fill={C.dim} opacity="0.6" />
                                  </svg>
                                  {item.soundName}
                                </span>
                              )}
                        ...
                        >
                          {item.likes >= 1_000_000
                            ? `${(item.likes / 1_000_000).toFixed(1)}M`
                            : item.likes >= 1_000
                              ? `${(item.likes / 1_000).toFixed(1)}K`
                        ...
                        >
                          {item.comments >= 1_000_000
                            ? `${(item.comments / 1_000_000).toFixed(1)}M`
                            : item.comments >= 1_000
                              ? `${(item.comments / 1_000).toFixed(1)}K`
                        ...
                        >
                          {item.shares >= 1_000_000
                            ? `${(item.shares / 1_000_000).toFixed(1)}M`
                            : item.shares >= 1_000
                              ? `${(item.shares / 1_000).toFixed(1)}K`
