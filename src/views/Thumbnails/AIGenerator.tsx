          ...
          {aiReferenceImage ? (
            ...
            <div style={{ width: '100%', aspectRatio: `${canvasW}/${canvasH}`, background: canvasBg, borderRadius: 12, border: `1px solid ${C.border}`, position: 'relative', overflow: 'hidden', marginBottom: 16 }}>
              {els.map((el) => {
                ...
                if (el.type === 'image') return <img key={el.id} src={el.src} alt={t('thumbs.ai.imageAlt')} decoding="async" style={{ position: 'absolute', left: el.x / canvasW * 100 + '%', top: el.y / canvasH * 100 + '%', width: el.w / canvasW * 100 + '%', height: el.h / canvasH * 100 + '%', opacity: el.opacity, borderRadius: el.borderR, objectFit: 'cover' }} />;
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {aiResults.map((r) => (
                  <div key={r.id} style={{ width: isMobile ? '100%' : 'calc(50% - 5px)', aspectRatio: '16/9', background: C.card, borderRadius: 10, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', position: 'relative', overflow: 'hidden' }}>
                    {r.url ? (
                      <img src={r.url} alt={r.label} loading="lazy" decoding="async" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : (
                      <>
