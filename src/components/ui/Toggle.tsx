...
export const Toggle = memo(function Toggle({ on, onChange, label, ck }: ToggleProps) {
  ...
  return (
    <div role="switch" aria-checked={on} aria-label={label} tabIndex={0} onClick={onChange} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onChange(); } }} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', padding: '5px 8px', borderRadius: 7, background: on ? `${col}12` : C.card, border: `1px solid ${on ? col + '33' : C.border}` }}>
