...
export const FrameSlot = memo(function FrameSlot({ label, has, onClick }: FrameSlotProps) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      style={{ flex: 1, borderRadius: 8, border: `1.5px dashed ${has ? C.green + '55' : C.border}`, background: has ? `${C.green}08` : C.card, padding: '10px 6px', textAlign: 'center', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, fontFamily: 'inherit' }}
    >
